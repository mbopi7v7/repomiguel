...
btnMostrar.addEventListener('click',btnMostrarClick);
function btnMostrarClick(){  
    console.log('Mostrando datos del localStorage desde btnMostrarClick');  
    /*
    const eventosData=localStorage.getItem('eventos_data');
    const eventosCount=localStorage.getItem('eventos_count');
    console.log(`Eventos Data: ${eventosData}`);
    console.log(`Eventos Count: ${eventosCount}`); */
    let eventosData=JSON.parse(localStorage.getItem('eventos_data'));
    console.log(`Eventos Data (parsed):`, eventosData);
    let contentidoEventos="";
    eventosData.forEach(evento => {
        //console.log(`Evento: ${evento.nombre}, Fecha: ${evento.fecha}, Lugar: ${evento.lugar}`);
        contentidoEventos+=`<tr><td>${evento.titulo}</td><td>${evento.lugar}</td><td>${evento.fecha}</td></tr>`;
    });
    tbody.innerHTML=contentidoEventos;
